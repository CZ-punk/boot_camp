...
//        food2.getUserList().add(user); // 외래 키(연관 관계) 설정
        ...
        // User 와 Food 는 정상적으로 영속화되었다. ( 1차 캐시 존재 )
        // 하지만, Tx 가 끝나지 않아 Orders table 은 전혀 반영되지 않은 상태
        // 그러므로 1차 캐시에 존재하는 User 정보를 통해 foodList 정보가 반영되지 않은 상태
        // FoodList 는 비어있는 상태이기 때문에 아무것도 출력 X

        // FK 의 주인이 아닌 User 부분 food 정보를 넣어준게 아무것도 없다.
        // Tx 종료 후면 반영되지만, 그 이전에 같은 비즈니스 로직에서는 User 에서 Food 정보를 추출 불가능하다.
...
//        food2.addUserList(user);
        ...
        // 해당 로직에서는 user 에 대한 food 정보가 찍히는 이유:
        // food.addUserList method 를 통해 food 에 user 에 대한 정보를 넣음과
        // 동시에 User 에 food 정보도 세팅해주는 편의 기능 메서드를 만들어서 구현했기 때문
